module.exports = {
  ...
  run: async function (lang) {
    if (this.options.getSubcommand() == 'song') {
      ...
      this.sendEmbed(lang(player.queue.repeatMode == 0 ? 'song.disabled' : 'song.enabled'), { asEmbed: true });
    }

    await this.musicPlayer.setRepeatMode(this.musicPlayer.repeatMode == 0 ? 2 : 0);
    this.sendEmbed(lang(player.queue.repeatMode == 0 ? 'queue.disabled' : 'queue.enabled'), { asEmbed: true });
  }
};