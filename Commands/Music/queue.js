...
module.exports = new Command({
  ...
  run: async player => {
    ...
    const embed = new MessageEmbed({
      ...
      colors: colors.discord.BURPLE
    });

    embed.addField('Current Song', `[${song.name.length <= 75 ? song.name : song.name.substring(0, 75) + '...'}](${song.url}) by ${song.uploader.name} \`${player.queue.formattedCurrentTime}\` / \`${remainingSongTime}\``);
