...
module.exports = async function errorHandler(err, message, lang) {
  ...
  const msg = await message.editReply({ embeds: [embed], components: [comp] });
  ...
  collector.on('collect', async button => {
    ...
    try {
      ...
      message.editReply(lang('events.errorHandler.reportFail', err?.response.statusText || 'unknown error'));
      this.error(err.stack);
    }
  });
